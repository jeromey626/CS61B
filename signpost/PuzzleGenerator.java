    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place currplace : sq.successors()) {
                    if (sq.connectable(model.get(currplace))) {
                        found = model.get(currplace);
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place currplace2 : sq.predecessors()) {
                    if (model.get(currplace2).connectable(sq)) {
                        found = model.get(currplace2);
                        if(found.sequenceNum() !=0 && sq.sequenceNum() != 0){
                            nFound = 1;
                            found = model.get(currplace2);
                        }
                        nFound = nFound + 1;
