...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int f = 0; f < _width; f++) {
            for (int d = 0; d < _height; d++) {
                if (solution[f][d] != last + 1) {
                    _solnNumToPlace[solution[f][d]] = Place.pl(f, d);
                }
            }
        }
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                int numSequence = solution[i][j];
                int seqalphax, seqbetay;
                if (numSequence == _width * _height) {
                    _board[i][j] = new Sq(i, j, _width * _height, true, 0, 0);
                } else if (numSequence == 1) {
                    _board[i][j] = new Sq(i, j, 1,
                            true, dirOf(i, j,
                            solnNumToPlace(2).x,
                            solnNumToPlace(2).y), 0);
                } else {
                    seqalphax = solnNumToPlace(numSequence + 1).x;
                    seqbetay = solnNumToPlace(numSequence + 1).y;
                    _board[i][j] = new Sq(i, j, 0,
                            false, dirOf(i, j, seqalphax, seqbetay),
                            -1);
                }
            }
        }
        ...
        for (int j = 1; j <= last; j++) {
            if (_solnNumToPlace[j] == null) {
                throw new
                        IllegalArgumentException(
                                "the sequence number isn't present");
            }
        }
        for (Sq square: _allSquares) {
            if (square._sequenceNum != last) {
                square._successors =
    ...
    /** A helper function to reduce length of model method.
     * @param last2 is the number last initialized the above model method */
    public void helperfunc1(int last2) {
        for (Sq square: _allSquares) {
            for (Sq sqdos: _allSquares) {
                square._predecessors = new PlaceList();
                if (square._sequenceNum != 1 && sqdos._sequenceNum != last2) {
                    for (Place p1 : _allSuccessors[sqdos.x][sqdos.y]
                            [sqdos.direction()]) {
                        if (p1.x == square.x && p1.y == square.y) {
                            square._predecessors.add(sqdos.pl);
                        }
                    }
                }
            }
    ...
    Model(Model model) {
        ...
        for (Sq square: _allSquares) {
            for (Sq othersquare: _allSquares) {
                if (square._sequenceNum == othersquare._sequenceNum - 1) {
                    square._successor = othersquare;
                    othersquare._predecessor = square;
                }
            }
        }
        ...
        for (Sq square: _allSquares) {
            Sq copycat = square;
            while (copycat._predecessor != null) {
                copycat = copycat._predecessor;
            }
            square._head = copycat;
        }
    }
    ...
    void init(int width, int height) {
        ...
        PlaceList [][][] allSuccessors = Place.successorCells(width, height);
    }
    ...
    boolean autoconnect() {
        ...
        for (Sq sq: _allSquares) {
            for (Sq sq2: _allSquares) {
                if (sq._sequenceNum == sq2._sequenceNum - 1
                        && sq._sequenceNum != 0
                    && sq2._sequenceNum != 0
                        && sq.connectable(sq2)) {
                    sq.connect(sq2);
    ...
    void solve() {
        for (int q = 0; q < _board.length; q++) {
            for (int w = 0; w < _board[0].length; w++) {
                get(q, w)._sequenceNum = _solution[q][w];
            }
        }
        _unconnected = 0;
    }
    ...
    private int arrowDirection(int x, int y) {
        int seqnum = _solution[x][y];
        int theway = 0;
        if (seqnum == _width * _height) {
            return 0;
        } else {
            theway = dirOf(x, y,
                    solnNumToPlace(seqnum + 1).x,
                    solnNumToPlace(seqnum + 1).y);
            return theway;
        }
    }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0) {
                Sq temppointer = this;
                while (temppointer._successor != null) {
                    if (temppointer._successor.sequenceNum() == 0) {
                        if (temppointer._successor == s1) {
                            releaseGroup(temppointer._successor._group);
                        }
                        temppointer._successor._sequenceNum =
                                temppointer.sequenceNum() + 1;
                    }
                    temppointer = temppointer._successor;
                }
            } else if (s1.sequenceNum() != 0) {
                Sq pointerbeta = s1;
                while (pointerbeta._predecessor != null) {
                    if (pointerbeta._predecessor.sequenceNum() == 0) {
                        if (pointerbeta._predecessor == this) {
                            releaseGroup(pointerbeta._predecessor._group);
                        }
                        pointerbeta._predecessor._sequenceNum =
                                pointerbeta.sequenceNum() - 1;
                    }
                    pointerbeta = pointerbeta._predecessor;
                }
            }

            Sq header = this;
            while (header != null) {
                if (header._predecessor == null) {
                    this._head = header;
                }
                header = header._predecessor;
            }

            Sq headerpoint = this;
            while (headerpoint != null) {
                headerpoint._head = this._head;
                headerpoint = headerpoint._successor;
            }
            if (s1._group == -1 && this._group == -1) {
                this._head._group = joinGroups(-1, -1);
            }
            return false;
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                if (!hasFixedNum() && _predecessor == null) {
                    _group = -1;
                } else {
                    boolean gotfixed = false;
                    Sq goingwayback = this;

                    while (!gotfixed && goingwayback != null) {
                        gotfixed = goingwayback.hasFixedNum();
                        goingwayback = goingwayback.predecessor();
                    }
                    if (!gotfixed) {
                        goingwayback = this;
                        int newnumgroup = newGroup();
                        while (goingwayback != null) {
                            goingwayback._sequenceNum = 0;
                            goingwayback._group = newnumgroup;
                            goingwayback = goingwayback.predecessor();
                        }
                    }
                }
