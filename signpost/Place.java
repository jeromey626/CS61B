...
class Place {
    ...
    static PlaceList[][][] successorCells(int width, int height) {
        ...
        for (int alp = 0; alp < width; alp++) {
            for (int be = 0; be < height; be++) {
                for (int dirpoint = 0; dirpoint < 9; dirpoint++) {

                  M[alp][be][dirpoint] = new PlaceList();
                    M[alp][be][0] = new PlaceList();
                    _places[alp][be] = pl(alp, be);

                    for (int charlie = 0; charlie < width; charlie++) {
                        for (int delta = 0; delta < height; delta++) {
                            if (dirpoint == dirOf(alp, be, charlie, delta)) {
                                M[alp][be][dirpoint].add(pl(charlie, delta));
                                M[alp][be][0].add(pl(charlie, delta));
                            }
                        }
                    }
                }
